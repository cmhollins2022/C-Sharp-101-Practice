...
With Object Oriented Programming (OOP), we can classify objects with various attributes. We can make a ```class```, along with ```public``` or ```private``` attributes. Again, it is necessary to explicitly declare the variable type. Utilize the ```ToString``` method to 
